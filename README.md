...
Sweat equity is modeled as a discrete project completed in a selected ownership year. The primary calculator asks only for total out-of-pocket cost, completion year, labor hours, and an optional hourly time value. It adaptively searches for the immediate post-project market-value increase required for the entire purchase to beat renting in at least 50% of simulated futures. That threshold calculation does not use or require an appraisal estimate.

An optional comparison section accepts conservative, most-likely, and optimistic estimates of the immediate value added. Those estimates are drawn once per path from a triangular distribution, recognized at the start of the selected completion year, and appreciated with the home afterward. Property tax, insurance, maintenance, and eventual selling costs automatically apply to the enlarged home value. The interface is prefilled with an opt-in Boston kitchen-remodel example: $20,000 of total cash cost, 750 hours of labor valued at $40 per hour, and a $15,000/$30,000/$45,000 value range in year 2. The project remains disabled in the main simulation by default so the unchanged baseline is still a pure buy-versus-rent comparison.
...
Select **Calculate required uplift** to run common market paths, bracket the 50% crossing, and refine it with binary search. The interface reports separate cash-only and time-adjusted answers, then compares the optional appraisal range with that threshold. A focused probability curve shows how the purchase decision changes as remodeling creates more value. This threshold is not the remodel's own break-even point or an appraisal: it is the value that comparable sales or a subject-to-completion appraisal would need to support for buying to win half the simulated paths.
